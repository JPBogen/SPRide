namespace AffinitySimulation
{
    ...
    */
    ...
    export function Calculate_KD(kon : number, kdis : number) : number
    {
        return kdis/kon; 
    }

    //calculates the Halflife based on kdis
    export function Calculate_Halflife(kdis : number) : number
    {
    ...
    export function Simulation_MCK(cycles: number, dilutionfactor : number, kon: number, kdis: number, maxconc: number, AssociationTime : number, DissociationTime : number) : XYValue[]
    {
        ...
        for (let i = 0; i < cycles; i++)
        {
            ...
            var set : XYValue[] = Generate_Binding_Simulation_MCK(kon, kdis, maxconc * (1/dilutionfactor) ** (i), AssociationTime, DissociationTime);
    ...
    export function Simulation_SCK(associaction_cycles: number, dilutionfactor : number, kon: number, kdis: number, maxconc: number, AssociationTime : number, DissociationTime : number, DissociationInBetween : boolean = false, DissociationInBetweenTime? : number) : XYValue[]
    {
        ...
        for (let i = 0; i < associaction_cycles; i++)
        {
            ...
            var Association_Y_Values : number[] = Calculate_AssociationCurve(kon, kdis, maxconc * (1/dilutionfactor) ** (associaction_cycles - i), AssociationTime, lastValue);
    ...
    export function Simulate_SteadyState(KD : number, conc: number, numberOfDatapoints? : number) : XYValue[]
    {
    ...
    function Generate_Binding_Simulation_MCK(kon: number, kdis: number, conc: number, AssociationTime : number, DissociationTime : number) : XYValue[]
    {
        //Calcuclates Association curve
        var Association_Y_Values : number[] = Calculate_AssociationCurve(kon, kdis, conc, AssociationTime);

        //Calculates dissociation curve. The start of the dissociation (Ye) is the last value in the association curve
        var Dissociation_Y_Values : number[] = Calculate_DissociationCurve(Association_Y_Values[AssociationTime-1], kdis, DissociationTime);
    ...
    function Calculate_Req(kon : number, kdis: number, AnalyteConc : number, Rmax? : number) : number
    {
    ...
    function Calculate_Req_SteadyState(KD : number, AnalyteConc : number, Rmax? : number) : number
    {
        //if no Rmax is given, 100 is used (100% or Rmax)
        if (Rmax == undefined)
        {
    ...
    function Calculate_kobs(kon : number, kdis: number, AnalyteConc : number) : number
    {
        return kon*AnalyteConc+kdis;
    }

    //Calculates an association curve. Agnostic of SCK or MCK
    function Calculate_AssociationCurve(kon : number, kdis: number, AnalyteConc : number, timeframe : number, YO? : number) : number[]
    {
        ...
        //Creates empty dataset
        var assosication_curve : number[] = []; 

        //calculates Req and kobs values
        var Req : number = Calculate_Req(kon, kdis, AnalyteConc);
        var kobs : number = Calculate_kobs(kon, kdis, AnalyteConc);
    ...
    function Calculate_DissociationCurve(Ye: number, kdis: number, timeframe : number) : number[]
    {
        //Create empty dataset
        var disscoiation_curve : number[] = [];
    ...
    export function Theta(kon : number, kdis : number, conc : number, theta? : number)
    {
    ...
    export function GibbsEnergy(KD : number) : number
    {
    ...
    export function Thetaeq(KD : number, conc : number) : number
    {
        return (conc)/(KD + conc);
    }
    ...
    // #region Dataset calculation

    //Defines dataset required for plotting using Chart.js
    ...
    function Generate_Datasets(curve: number[], start: number, end: number): XYValue[]
    {
        //creates empty dataset
        var xyValues: XYValue[] = [];
